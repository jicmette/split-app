...
export default function DashboardScreen() {
      ...
      const loadData = async () => {
        ...
        try {
          ...
          if (docSnap.exists()) {
            ...
            // Load Bills and merge checked bills
            const loadedBills: Bill[] = data?.bills || [];
            setBills(loadedBills);

            const savedPlan: PaycheckPlanItem[] = data?.paycheckPlan || [];
            const freshPlan = calculatePaycheckPlan(
              data?.payFrequency,
              loadedBills
            );
            const finalPlan = freshPlan.map((freshItem) => {
              const savedItem = savedPlan.find(
                (saved) => saved.billId === freshItem.billId
              );
              if (savedItem) {
                return { ...freshItem, isSaved: savedItem.isSaved };
              }
              return freshItem;
            });
            setPaycheckPlan(finalPlan);
          }
        } catch (e) {
  ...
  if (!userProfile) {
    ...
    return (
      <SetupForm onSave={handleSaveProfile} userNameFromGoogle={userName} />
    );
  }
